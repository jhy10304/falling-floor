...
GameObject *SimpleGameObjectFactory::randomGameObject() {
  int x = distX(engine);
  int y = distY(engine);
  return new GameObject(IconFactory::NxMColor(Size(1, 1), BLUE),
                        Position{x, y});
}
GameObject *SimpleGameObjectFactory::randomGameObject() {
  int x = distX(engine);
...
GameObject *SimpleGameObjectFactory::randomGameObjectpara(const Position& playerPos) {
  Position pos;
  do {
    pos = Position{distX(engine), distY(engine)};
} while (pos == playerPos);  // 比較位置是否和玩家重疊
